...
void shutdown(void);
bool startup(void);
void input_number(char ch);
bool check_number(char ch); 
void clean_input(void);
bool check_won(void);
void paint_all_numbers(void);
...
main(int argc, char *argv[])
{
    ...
    do
    {
        ...
        // log input (and board's state) if any was received this 
        // iteration
        if (ch != ERR)
            log_move(ch);
    }
...
draw_borders(void)
{
    ...
    // draw header
    char header[maxx+1];
    sprintf(header, "%s by the big %s", TITLE, AUTHOR);
...
void
input_number(char ch)
{
   if(g.board[g.y][g.x] == 0 || g.board[g.y][g.x] > 100){
       if(!check_number(ch)){
           show_banner("Posição inválida");
       }else{
           hide_banner();
		   mvaddch(g.top + g.y + 1 + g.y/3, g.left + 2 + 2*(g.x + g.x/3), ch);
		   refresh();
		   int num = ch - '0';
		   g.board[g.y][g.x] = 100 + num;
		   if(check_won()){
			   show_banner("Parabéns");
			   paint_all_numbers();
		   }
	   }
   }    
...
bool
check_number(char ch)
{
    int num = ch - '0';
    for(int i = 0; i < 9; i++){
	    if(g.y != i)
	        if(g.board[i][g.x] == num)
	            return false; 
	    if(i != g.x)
	        if(g.board[g.y][i] == num)
	            return false; 
	}
	int posicao = g.x + 1;
	int posicaoX;
	if(posicao >= 0 && posicao < 9){
		if(g.board[g.y][posicao] == num)
			return false;
	}
	posicao = g.x - 1;
	if(posicao >= 0 && posicao < 9){
	...
	posicao = g.x - 1;
	if(posicao >= 0 && posicao < 9){
		if(g.board[posicao][posicao] == num)
			return false;
	}
	posicao = g.x - 1;
	posicaoX = g.x + 1;
	if(posicao >= 0 && posicao < 9 && posicaoX >= 0 && posicaoX < 9){
		if(g.board[posicao][posicaoX] == num)
			return false;
	}
	posicao = g.x - 1;
	posicaoX = g.x + 1;
	if(posicao >= 0 && posicao < 9 && posicaoX >= 0 && posicaoX < 9){
		if(g.board[posicaoX][posicao] == num)
			return false;
	}
	posicaoX = g.x + 1;
	if(posicaoX >= 0 && posicaoX < 9){
		if(g.board[posicaoX][posicaoX] == num)
			return false;
	}
